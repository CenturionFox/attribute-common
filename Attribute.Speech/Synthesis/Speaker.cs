...
using Attribute.Common.Framework;
...
namespace Attribute.Speech.Synthesis
{
    ...
    /// </summary>
    [DataContract,
     DebuggerDisplay("Speaker[Name:{SpeakerName},Voice:{VoiceName},ComponentState:{ComponentState},CustomSynthesizer:{HasCustomSynthesizer}]"),
     Serializable,
     XmlRoot(ElementName = "speaker")]
    public sealed class Speaker : ISpeaker, INotifyPropertyChanged, IAttributeComponent
    {
        #region [-- CONSTRUCTORS --]
        ...
        /// <summary>
        ///     Stops the speaker's currently queued prompt.
        /// </summary>
        /// <remarks>Will not set the component to an errored state.</remarks>
        public void Stop()
        {
            if (this.HasCustomSynthesizer)
            {
                if (this.Synthesizer.State == SynthesizerState.Speaking)
                {
                    var currentlySpokenPrompt = this.Synthesizer.GetCurrentlySpokenPrompt();

                    if (currentlySpokenPrompt != null)
                    {
                        this.Synthesizer.SpeakAsyncCancel(currentlySpokenPrompt);
                    }
                }
            }
        }
            ...
            set
            {
                if (value != this._synthesizer)
                {
                    ...
                    if (this._synthesizer != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(this._voiceName))
                        {
                            try
                            {
                                this._synthesizer.SelectVoice(this._voiceName);
                            }
                            catch (Exception ex)
                            {
                                ex.Data.Add(nameof(this.VoiceName), this._voiceName);
                                this.Errored?.Invoke(this, new UnhandledExceptionEventArgs(ex, false));
                            }
                        }
                        else
                        {
                            // NOTE: Do not set voice as this will lead to an unnecessary call to SpeechSynthesizer.SetVoice(String)
                            this._voiceName = this._synthesizer.Voice.Name;
                        }
                    }
                }
            }
        ...
        public string VoiceName
        {
            ...
            set
            {
                if (!value.Equals(this._voiceName))
                {
                    ...
                    if (this.Synthesizer != null && !string.IsNullOrEmpty(this._voiceName))
                    {
                        try
                        {
                            this.Synthesizer.SelectVoice(this._voiceName);
                        }
                        catch (Exception ex)
                        {
                            ex.Data.Add(nameof(this.VoiceName), this._voiceName);
        ...
        /// </value>
        public AttributeComponentState ComponentState => this.IsErrored ? AttributeComponentState.Errored : AttributeComponentState.Normal;
